...
[個別銘柄を名指しした場合]
  ...
  → スイング売買ルール(swing_rules.py)で決定論的に判定(Geminiは判定に使わない):
    ├ 未保有: 買いルールの全条件チェック → 買い候補(注文レシピ付き) or
    │         様子見(満たしていない条件を列挙)
    └ 保有中(台帳に登録あり): 建値ベースの出口判定
              (利確/損切りライン・20営業日の時間切れ)→ 売り時 or 様子見

[取引台帳(Phase 10、Supabase設定時)]
  「50万入金した」「7203を1880円で100株買った」「余力いくら?」「さっきの取り消して」
  → 台帳テーブルに追記し、余力・保有(平均取得単価)を復唱返信
  → 夜間バッチの候補予算が固定30万円→実余力に切り替わり、
...
- Gemini の無料枠は **20リクエスト/日(モデル毎、2026-07時点)**。かつては flash-lite の
  ...
  - LINE返信: 条件解析の1回+スクリーニング時のみ解説文生成で+1回。
    個別銘柄の売買判断と取引台帳はルールベースでGemini追加なし
...
スイング候補の予算が固定30万円から**実際の余力**に切り替わる。
